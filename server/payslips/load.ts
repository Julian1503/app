/** Payslips: lectura desde la base y parseo desde los PDF del disco.
 *
 *  Las dos mitades estan separadas a proposito. El servidor solo lee la tabla
 *  (`loadPayslips`); leer y parsear los PDF es un trabajo de importacion que
 *  corre a mano con `npm run payslips:import` y no necesita estar en el camino
 *  de cada request. Por eso tampoco hay mas cache por mtime: se parsea cuando
 *  vos lo pedis, no cuando alguien abre el reporte. */
...
/** Lo que consume la app: los payslips ya parseados que hay en la base. */
...
/** Parsea la carpeta de PDF entera. Es la entrada del importador: devuelve el
 *  set completo, ya deduplicado y con los sleepovers inferidos, listo para
 *  escribir en la base. */
